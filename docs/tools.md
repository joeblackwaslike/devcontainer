...
Comprehensive reference for everything installed in the `ghcr.io/joeblackwaslike/devcontainer`
image. Each section lists the install method, version strategy, and purpose.
**Update this file whenever you add, remove, or upgrade a tool in `image/Dockerfile`.**

---

## System packages (apt / Aptfile)

Installed at image build time via `seatgeek/bash-aptfile` running against `image/Aptfile`.
...
To upgrade: update the PGDG source list in `image/Dockerfile` and rebuild.
...
|---|---|---|
| 1Password CLI | `op` | `op run` wraps MCP server commands with secrets from 1Password vaults |
...
To add a new runtime: add `asdf plugin add <name> && asdf install <name> <version>` to
`image/Dockerfile`, add a row here, and update the changelog.
...
|---|---|---|
| uv | install script → `/usr/local/bin` | Fast Python package/project manager |
...
| volta | install script → `/usr/local/share/volta` | Supplemental JS toolchain for volta-pinned repos |
...
To add a cargo tool: add to the `cargo install` block in `image/Dockerfile` and add a row here.
...
| `@devcontainers/cli` | `devcontainer` | Dev Containers CLI (used by `just init` and `just publish-templates`) |
...
Installed via install script to `/usr/local/bin/oh-my-posh`.
Theme defined in `image/.mytheme.omp.yaml` (copied into the image at build time).

---

## Lifecycle scripts

Baked into the image at `/usr/local/share/devcontainer/scripts/` during Docker build
(see `image/scripts/` in this repo). Referenced directly by `devcontainer.json` lifecycle
hooks — no per-project script copying required.

| Script | Hook | Purpose |
|---|---|---|
| `setup.sh` | `postCreateCommand` | Once on create: mkcert CA install, `~/.claude/skills` symlink |
| `discover-deps.sh` | `postAttachCommand` | Every attach: auto-install project dependencies |
...
| 2026-06-01 | Restructured repo: build context moved to `image/`, scripts baked into image, init-project.sh replaced by Dev Container Templates in `src/` |
